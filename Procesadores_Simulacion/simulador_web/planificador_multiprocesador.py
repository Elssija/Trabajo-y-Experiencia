...
class Reloj(threading.Thread):
    ...
    def __init__(self, procesadores):
        #recibiendo los datos para la clase hija.
        super().__init__(name="Reloj")
        self._tiempo_sistema = 0
        self._lock = threading.Lock() #Bloquea el paso de Hilos. "Candado"
    ...
    def detener(self):
        self._detener.set() #Detiene el hilo esto retorna un false
...
class Procesador(threading.Thread):
    ...
    def run(self):
        ...
        #esto claramente mediante la utilizacion de semaforos
            ...
            #aqui dependen de la estragia asignado seran lo ticks del procesador
            #FIFO = Ninguno
            #ROUND ROBIN= quantum
            ...
            if hilo.esta_terminado():
                ...
                with self.lock_terminados: #trychatch reducido
                ...
                if cantidad_terminados >= self.total_hilos:
                    ...
                    self.evento_fin.set() #false bandera
            else:
                hilo.estado = "espera"
